...
# --- HINTERGRUND: ANIMIERTES WASSERBAD MIT LUFTBLASEN ---
st.markdown(
    """
    <style>
...
def sende_protokoll_email(bericht_text):
    # WICHTIG: Hier deine E-Mail-Daten eintragen!
    smtp_server = "smtp.gmail.com"  # Falls du Gmail nutzt, sonst z.B. smtp.strato.de etc.
    port = 465  # SSL Port
    absender_email = "DEINE_ANLAGEN_EMAIL@gmail.com" 
    passwort = "DEIN_SMTP_APP_PASSWORT" # Kein normales Passwort, sondern ein "App-Passwort"
...
if uv_anlage and pumpen and bandfilter_bereit and salz_geprueft:
    st.success("✅ Morgen-Check-up abgeschlossen!")
    st.divider()
    ...
    with tab1:
        ...
        if bedarf_kg1 > 0:
            st.error(f" Nachdosieren: {round(bedarf_kg1, 1)} kg ({round(bedarf_kg1/1.09, 1)} L)")
    ...
    with tab4:
        ...
        if bedarf_kg4 > 0:
            st.error(f" Nachdosieren: {round(bedarf_kg4, 2)} kg ({round(bedarf_kg4/1.09, 2)} L)")
    ...
    with tab6:
        ...
        # Beispielhafte historische Daten für die Visualisierung vor Ort
        historische_daten = {
        ...
        st.write("📈 Verlauf der Entfettungs-Konzentration:")
        ...
        if st.button("🚀 Analyse absenden & E-Mail verschicken"):
            if mitarbeiter_name == "":
                ...
                # Text für die E-Mail zusammenbauen
...
Deine Vorbehandlungs-App"""
                ...
                with st.spinner("E-Mail wird gesendet..."):
                    ...
                    if erfolg:
                        st.success("🎉 Protokoll erfolgreich per E-Mail an den Chef gesendet!")
                    else:
                        st.error("❌ E-Mail konnte nicht gesendet werden. Überprüfe die Zugangsdaten im Code.")
else:
